...
function minErr(module) {
	return function() {
		...
		message = message + '\nhttp://errors.angularjs.org/1.2.25/' + (module ? module + '/' : '') + code;
 ...
 * var app = angular.module('multi-bootstrap', []).controller('BrokenTable', function($scope) {
 ...
 *     expect(element.all(by.css('td')).count()).toBe(9);
 * });
 * </file>
 * </example>
 *
		...
		return function(name, requires, configFn) {
			...
			return ensure(modules, name, function() {
				...
				var moduleInstance = {
					 ...
					 *       return function(element) {// cancellation
 ...
 * - `scope()` - retrieves the {@link ng.$rootScope.Scope scope} of the current element or its parent.
 ...
 * function is invoked. There are three ways in which the function can be annotated with the needed dependencies.
 *
 * # Argument names
 *
 ...
 *
 *   // Therefore
 *   expect(injector.annotate(['$compile', '$rootScope', function(obfus_$compile, obfus_$rootScope) {}])).toEqual(['$compile', '$rootScope']);
 * ```
 *
 ...
 * are constructor functions, whose instances are responsible for "providing" a factory for a service.
 *
 ...
 * which lets you specify whether the {@link ng.$log $log} service will log debug messages to the console or not.
 *
 ...
 *
 * You should use {@link auto.$provide#service $provide.service(class)} if you define your service as a type/class.
 *
 * @param {string} name The name of the instance.
 ...
 * provider's `$get` property is a factory function that takes no arguments and returns the **value service**.
 *
...
var $AnimateProvider = ['$provide', function($provide) {
	...
	this.$get = ['$timeout', '$$asyncCallback', function($timeout, $$asyncCallback) {
		 ...
		 * ngAnimate module page} as well as the {@link ngAnimate.$animate ngAnimate $animate service page}.
		 */
		return {
			/**
			 *
			 ...
			 * @name $animate#enter
			 * @kind function
			 * @description Inserts the element into the DOM either after the `after` element or within the `parent` element. Once complete, the done() callback will be fired (if provided).
			 * @param {DOMElement} element the element which will be inserted into the DOM
			 * @param {DOMElement} parent the parent element which will append the element as a child (if the after element is not present)
			 * @param {DOMElement} after the sibling element which will append the element after itself
			 ...
			 * @name $animate#leave
			 * @kind function
			 * @description Removes the element from the DOM. Once complete, the done() callback will be fired (if provided).
			 ...
			 *
			 * @param {DOMElement} element the element which will be moved around within the DOM
			 * @param {DOMElement} parent the parent element where the element will be inserted into (if the after element is not present)
			 * @param {DOMElement} after the sibling element where the element will be positioned next to
...
function Browser(window, document, $log, $sniffer) {
	...
	self.onUrlChange = function(callback) {
		...
		if (!urlChangeInit) {
			...
			if ($sniffer.history) {// html5 history api - popstate event
				jqLite(window).on('popstate', fireUrlChange);
			}
			
			if ($sniffer.hashchange) {// hashchange event
...
function $CacheFactoryProvider() {
	this.$get = function() {
		...
		function cacheFactory(cacheId, options) {
			 ...
			 *
			 * ```js
			 *  angular.module('superCache').factory('superCache', ['$cacheFactory', function($cacheFactory) {
 ...
 * the template loading is asynchronous the compilation/linking is suspended until the template is loaded.
 *
 ...
 *   object which allows the directives to use the attributes object as inter directive communication.
 *
...
function $CompileProvider($provide, $$sanitizeUriProvider) {
	...
	this.$get = ['$injector', '$interpolate', '$exceptionHandler', '$http', '$templateCache', '$parse', '$controller', '$rootScope', '$document', '$sce', '$animate', '$$sanitizeUri', function($injector, $interpolate, $exceptionHandler, $http, $templateCache, $parse, $controller, $rootScope, $document, $sce, $animate, $$sanitizeUri) {
		...
		Attributes.prototype = {
			 ...
			 * compilation. The observer is then invoked whenever the interpolated value changes.
			 *
			 * @param {string} key Normalized key. (ie ngAttribute) .
			 * @param {function(interpolatedValue)} fn Function that will be called whenever the interpolated value of the attribute changes.
		...
		function compile($compileNodes, transcludeFn, maxPriority, ignoreDirective, previousCompileContext) {
			...
			return function(scope, cloneConnectFn, transcludeControllers, parentBoundTranscludeFn) {
				assertArg(scope, 'scope');
				// important!!: we must call our jqLite.clone() since the jQuery one is trying to be smart
				// and sometimes changes the structure of the DOM.
				var $linkNode = cloneConnectFn ? JQLitePrototype.clone.call($compileNodes) /* IMPORTANT!!! */ : $compileNodes;
					...
					forEach(newIsolateScopeDirective.scope, function(definition, scopeName) {
						...
						case '=':
							...
							isolateScope.$watch(function() {
								...
								if (!compare(parentValue, isolateScope[scopeName])) {// we are out of sync and need to copy
									if (!compare(parentValue, lastValue)) {// parent changed and it has precedence
										isolateScope[scopeName] = parentValue;
									} else {// if the parent can be assigned then do so
				...
				while (linkQueue.length) {
					...
					if (beforeTemplateLinkNode !== beforeTemplateCompileNode) {
						...
						if (!(previousCompileContext.hasElementTranscludeDirective && origAsyncDirective.replace)) {
							linkNode = jqLiteClone(compileNode);// it was cloned therefore we have to clone as well
						}
						
						replaceWith(linkRootElement, jqLite(beforeTemplateLinkNode), linkNode);
						
						safeAddClass(jqLite(linkNode), oldClasses);// Copy in CSS classes from original node
					}
 ...
 *   </file>
 *   <file name="script.js">
 *     angular.module('documentExample', []).controller('ExampleController', ['$scope', '$document', function($scope, $document) {
...
function $HttpProvider() {
	...
	this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector', function($httpBackend, $browser, $cacheFactory, $rootScope, $q, $injector) {
		...
		forEach(responseInterceptorFactories, function(interceptorFactory, index) {
			 ...
			 * had to pick one.) But they are already reversed, so we can't use unshift, hence the splice.
			 */
		 ...
		 * an object representing the response. See the API signature and type info below for more details.
		 *
		 ...
		 * `transformRequest` and/or `transformResponse` properties of the configuration object passed into `$http`.
		 *
		 * # Caching
		 *
		 ...
		 *       ng-click="updateModel('JSONP', 'https://angularjs.org/greet.php?callback=JSON_CALLBACK&name=Super%20Hero')">
		 *       Sample JSONP
		 *     </button>
		 ...
		 * </file>
		 * <file name="script.js">
		 *   angular.module('httpExample', []).controller('FetchController', ['$scope', '$http', '$templateCache', function($scope, $http, $templateCache) {
		 *     $scope.method = 'GET';
		 ...
		 *     data = element(by.binding('data')),
		 *     fetchBtn = element(by.id('fetchbtn')),
		 *     sampleGetBtn = element(by.id('samplegetbtn')),
		 *     sampleJsonpBtn = element(by.id('samplejsonpbtn')),
		 *     invalidJsonpBtn = element(by.id('invalidjsonpbtn'));
		 *   
	...
	this.$get = ['$parse', '$exceptionHandler', '$sce', function($parse, $exceptionHandler, $sce) {
		...
		function $interpolate(text, mustHaveExpression, trustedContext) {
			...
			while (index < length) {
				...
				} else {// we did not find anything, so we have to add the remainder to the parts array
...
function $IntervalProvider() {
	this.$get = ['$rootScope', '$window', '$q', function($rootScope, $window, $q) {
		 ...
		 * move forward by `millis` milliseconds and trigger any functions scheduled to run in that time.
		 *
		 * <div class="alert alert-warning">
		 ...
		 * @param {number=} [count=0] Number of times to repeat. If not set, or 0, will repeat indefinitely.
		 ...
		 * <file name="index.html">
		 *   <script>
		 *     angular.module('intervalExample', []).controller('ExampleController', ['$scope', '$interval', function($scope, $interval) {
...
function LocationHtml5Url(appBase, basePrefix) {
	...
	this.$$rewrite = function(url) {
		...
		if ((appUrl = beginsWith(appBaseNoFile, url)) !== undefined) {
		...
		$rootElement.on('click', function(event) {
			...
			if (LocationMode === LocationHashbangInHtml5Url) {
				...
				if (href && href.indexOf('://') < 0) {// Ignore absolute URLs
					...
					if (href[0] == '/') {// absolute path - replace old path
						absHref = appBase + prefix + href;
					} else if (href[0] == '#') {// local anchor
						absHref = appBase + prefix + ($location.path() || '/') + href;
					} else {// relative path - join with current path
...
function ensureSafeObject(obj, fullExpression) {
	...
	if (obj) {
		...
		if (obj.document && obj.location && obj.alert && obj.setInterval) {// isWindow(obj)
			throw $parseMinErr('isecwindow', 'Referencing the Window in Angular expressions is disallowed! Expression: {0}', fullExpression);
		}
		if (obj.children && (obj.nodeName || (obj.prop && obj.attr && obj.find))) {// isElement(obj)
			throw $parseMinErr('isecdom', 'Referencing DOM nodes in Angular expressions is disallowed! Expression: {0}', fullExpression);
		}
		if (obj === Object) {// block Object so that we can't get hold of dangerous Object.* methods
...
function ensureSafeFunction(obj, fullExpression) {
	if (obj) {
		...
		if (obj === CALL || obj === APPLY || (BIND && obj === BIND)) {
...
function cspSafeGetterFn(key0, key1, key2, key3, key4, fullExp, options) {
	...
	return !options.unwrapPromises ? function(scope, locals) {// cspSafeGetter
		...
		return pathVal;
	} : function(scope, locals) {// cspSafePromiseEnabledGetter
		var pathVal = (locals && locals.hasOwnProperty(key0)) ? locals : scope,
			promise;
...
function getterFn(path, options, fullExp) {
	...
	if (options.csp) {
		...
		forEach(pathKeys, function(key, index) {
			...
			// we simply dereference 's' on any .dot notation but if we are first then we check locals first, and if so read it first
 ...
 *   specification](https://github.com/kriskowal/q/wiki/API-Reference#promisefinallycallback) for more information.
 *
...
function qFactory(nextTick, exceptionHandler) {
	 ...
	 * current promise, you have to "rethrow" the error by returning a rejection constructed via `reject`.
	 *
	 * ```js
	 *   promiseB = promiseA.then(function(result) {
...
function $RootScopeProvider() {
	...
	this.$get = ['$injector', '$exceptionHandler', '$parse', '$browser', function($injector, $exceptionHandler, $parse, $browser) {
		 ...
		 *         when unit-testing and having the need to override a default service.
		...
		Scope.prototype = {
			 ...
			 *         When creating widgets, it is useful for the widget to not accidentally read parent state.
			 *
			...
			$new: function(isolate) {
				...
				} else {// Only create a child scope class if somebody asks for one, but cache it to allow the VM to optimize lookups.
			 ...
			 *
			 * @param {function(newCollection, oldCollection, scope)} listener a callback function called when a change is detected.
			...
			$watchCollection: function(obj, listener) {
				var self = this,
					...
					newValue,
					// a shallow copy of the newValue from the last dirty-check run, updated to match newValue during dirty-check run
					oldValue,
			 ...
			 * the expression are propagated (uncaught). This is useful when evaluating Angular expressions.
			 *
			 * # Example
			 * ```js
			 ...
			 * registered listeners along the way. The event will stop propagating if one of the listeners cancels it.
			 *
...
function $SceDelegateProvider() {
	...
	this.$get = ['$injector', function($injector) {
		...
		function matchUrl(matcher, parsedUrl) {
			...
			return !!matcher.exec(parsedUrl.href);// definitely a regex. See adjustMatchers()
		}
		 ...
		 *
		 * @param {string} type The kind of context in which this value is safe for use. e.g. url, resourceUrl, html, js and css.
		 ...
		 * @param {*} maybeTrusted The result of a prior {@link ng.$sceDelegate#trustAs `$sceDelegate.trustAs`} call.
		 * @returns {*} The value the was originally provided to {@link ng.$sceDelegate#trustAs `$sceDelegate.trustAs`} if valid in this context. Otherwise, throws an exception.
		 */
 ...
 * ng.$sce#parseAsHtml $sce.parseAsHtml(binding expression)}. Here's the actual code (slightly simplified):
 *
 * ```
 ...
 * won't work on all browsers. Also, loading templates from `file://` URL does not work on some browsers.
 *
 ...
 *      characters: '`:`', '`/`', '`.`', '`?`', '`&`' and ';'. It's a useful wildcard for use in a whitelist.
 ...
 *      sub-domain could be matched against a `:` or literal `.` that was likely not intended. It
 ...
 *
 *   mySceApp.controller("myAppController", function($http, $templateCache, $sce) {
 *     var self = this;
 *     $http.get("test_data.json", {cache: $templateCache}).success(function(userComments) {
 *       self.userComments = userComments;
 *     });
 *     self.explicitlyTrustedHtml = $sce.trustAsHtml('<span onmouseover="this.textContent=&quot;Explicitly trusted HTML bypasses ' + 'sanitization.&quot;">Hover over this text.</span>');
 *   });
 * </file>
 *
 ...
 *       expect(element.all(by.css('.htmlComment')).first().getInnerHtml()).toBe('<span>Is <i>anyone</i> reading this?</span>');
 *     });
 *
 *     it('should NOT sanitize explicitly trusted values', function() {
 *       expect(element(by.id('explicitlyTrustedHtml')).getInnerHtml()).toBe('<span onmouseover="this.textContent=&quot;Explicitly trusted HTML bypasses ' + 'sanitization.&quot;">Hover over this text.</span>');
 *     });
 *   });
 * </file>
 * </example>
...
function $SceProvider() {
	 ...
	 *     trustAs is wrapping the given values into some type, this operation unwraps it when given such a value.
	 *
	 ...
	 *
	 *     getTrusted($sce.RESOURCE_URL, value) succeeding implies that getTrusted($sce.URL, value) will also succeed.
	 *
	...
	this.$get = ['$parse', '$sniffer', '$sceDelegate', function($parse, $sniffer, $sceDelegate) {
		...
		if (enabled && $sniffer.msie && $sniffer.msieDocumentMode < 8) {
			throw $sceMinErr('iequirks', [
				'Strict Contextual Escaping does not support Internet Explorer version < 9 in quirks mode.',
				'You can fix this by adding the text <!doctype html> to the top of your HTML document.',
		 ...
		 * that uses the provided value. See * {@link ng.$sce $sce} for enabling strict contextual escaping.
		 *
		 * @param {string} type The kind of context in which this value is safe for use. e.g. url, resource_url, html, js and css.
		 * @param {*} value The value that that should be considered trusted/safe.
		 * @returns {*} A value that can be used to stand in for the provided `value` in places
		 * where Angular expects a $sce.trustAs() return value.
		 */
		 ...
		 *
		 * @param {*} value The value to trustAs.
		 * @returns {*} An object that can be passed to {@link ng.$sce#getTrustedUrl $sce.getTrustedUrl(value)} to obtain the original value.
		 * (privileged directives only accept expressions that are either literal constants or are the return value of {@link ng.$sce#trustAs $sce.trustAs}.)
		 */
		 ...
		 *
		 * @param {*} value The value to trustAs.
		 * @returns {*} An object that can be passed to {@link ng.$sce#getTrustedResourceUrl $sce.getTrustedResourceUrl(value)} to obtain the original value.
		 * (privileged directives only accept expressions that are either literal constants or are the return value of {@link ng.$sce#trustAs $sce.trustAs}.)
		 */
		 ...
		 *
		 * @param {*} value The value to trustAs.
		 * @returns {*} An object that can be passed to {@link ng.$sce#getTrustedJs $sce.getTrustedJs(value)} to obtain the original value.
		 * (privileged directives only accept expressions that are either literal constants or are the return value of {@link ng.$sce#trustAs $sce.trustAs}.)
		 */
		 ...
		 * @returns {*} The value the was originally provided to {@link ng.$sce#trustAs `$sce.trustAs`} if valid in this context. Otherwise, throws an exception.
		 */
...
function $SnifferProvider() {
	this.$get = ['$window', '$document', function($window, $document) {
		...
		return {
			...
			// jshint +W018
			hashchange: 'onhashchange' in $window && (!documentMode || documentMode > 7),// IE8 compatible mode lies
...
// exactly the behavior needed here. There is little value is mocking these out for this service.
 ...
 *   <file name="index.html">
 *     <script>
 *       angular.module('windowExample', []).controller('ExampleController', ['$scope', '$window', function($scope, $window) {
 ...
 *   <file name="index.html">
 *     <script>
 *       angular.module('currencyExample', []).controller('ExampleController', ['$scope', function($scope) {
 ...
 *   <file name="index.html">
 *     <script>
 *     angular.module('numberFilterExample', []).controller('ExampleController', ['$scope', function($scope) {
 ...
 *       expect(element(by.binding("1288323623006 | date:'yyyy-MM-dd HH:mm:ss Z'")).getText()).toMatch(/2010\-10\-2\d \d{2}:\d{2}:\d{2} (\-|\+)?\d{4}/);
 ...
 *   <file name="index.html">
 *     <script>
 *       angular.module('limitToExample', []).controller('ExampleController', ['$scope', function($scope) {
 ...
 *   <file name="index.html">
 *     <script>
 *     angular.module('orderByExample', []).controller('ExampleController', ['$scope', function($scope) {
 ...
 *       <table class="friend">
 *         <tr>
 *           <th><a href="" ng-click="predicate = 'name'; reverse=false">Name</a> (<a href="" ng-click="predicate = '-name'; reverse=false">^</a>)</th>
 ...
 *   <file name="index.html">
 *     <script>
 *     angular.module('formExample', []).controller('FormController', ['$scope', function($scope) {
...
var inputType = {
	...
	*   <file name="index.html">
	*     <script>
	*     angular.module('textInputExample', []).controller('ExampleController', ['$scope', function($scope) {
	*       $scope.text = 'guest';
	*       $scope.word = /^\s*\w*\s*$/;
	*     }]);
	*     </script>
	*     <form name="myForm" ng-controller="ExampleController">
	*       Single word: <input type="text" name="input" ng-model="text" ng-pattern="word" required ng-trim="false">
	...
	* </example>
	 ...
	 *   <file name="index.html">
	 *     <script>
	 *       angular.module('numberExample', []).controller('ExampleController', ['$scope', function($scope) {
	 *         $scope.value = 12;
	 *       }]);
	 *     </script>
	 *     <form name="myForm" ng-controller="ExampleController">
	 ...
	 *   <file name="index.html">
	 *     <script>
	 *       angular.module('urlExample', []).controller('ExampleController', ['$scope', function($scope) {
	 ...
	 *       <span class="error" ng-show="myForm.input.$error.required">Required!</span>
	 *       <span class="error" ng-show="myForm.input.$error.url">Not valid url!</span>
	 *       <tt>text = {{text}}</tt><br/>
	 *       <tt>myForm.input.$valid = {{myForm.input.$valid}}</tt><br/>
	 *       <tt>myForm.input.$error = {{myForm.input.$error}}</tt><br/>
	 *       <tt>myForm.$valid = {{myForm.$valid}}</tt><br/>
	 ...
	 *   <file name="index.html">
	 *     <script>
	 *       angular.module('emailExample', []).controller('ExampleController', ['$scope', function($scope) {
	 ...
	 *   <file name="index.html">
	 *     <script>
	 *       angular.module('checkboxExample', []).controller('ExampleController', ['$scope', function($scope) {
	 *         $scope.value1 = true;
	 *         $scope.value2 = 'YES'
	 *       }]);
	 *     </script>
...
function textInputType(scope, element, attr, ctrl, $sniffer, $browser) {
	...
	var listener = function(ev) {
		...
		// necessary to revalidate even if the control's value is the same empty value twice in a row.
 ...
 *   <file name="index.html">
 *     <script>
 *     angular.module('inputExample', []).controller('ExampleController', ['$scope', function($scope) {
 ...
 *       userNameValid = element(by.binding('myForm.userName.$valid')),
 *       lastNameValid = element(by.binding('myForm.lastName.$valid')),
 *       lastNameError = element(by.binding('myForm.lastName.$error')),
 *       formValid = element(by.binding('myForm.$valid')),
 *       userNameInput = element(by.model('user.name')),
 *       userLastInput = element(by.model('user.last'));
 *     
 *     it('should initialize to model', function() {
 ...
 *
 * @property {Array.<Function>} $formatters Array of functions to execute, as a pipeline, whenever the model value changes.
 ...
 *   <file name="index.html">
 *     <script>
 *       angular.module('listExample', []).controller('ExampleController', ['$scope', function($scope) {
 ...
 *   <file name="index.html">
 *     <script>
 *     angular.module('valueExample', []).controller('ExampleController', ['$scope', function($scope) {
 ...
 *   </file>
 *   <file name="script.js">
 *     angular.module('bindHtmlExample', ['ngSanitize']).controller('ExampleController', ['$scope', function($scope) {
 ...
 *   names of the properties whose values are truthy will be added as css classes to the element.
 *
 ...
 *       expect(element(by.repeater('name in names').row(0).column('name')).getAttribute('class')).toMatch(/odd/);
 *       expect(element(by.repeater('name in names').row(1).column('name')).getAttribute('class')).toMatch(/even/);
 *     });
 *   </file>
 * </example>
 */
 ...
 * document; alternatively, the css rule above must be included in the external stylesheet of the application.
 *
 ...
 *         expect(container.element(by.model('settings.name')).getAttribute('value')).toBe('John Smith');
 *
 *       var firstRepeat = container.element(by.repeater('contact in settings.contacts').row(0));
 *       var secondRepeat = container.element(by.repeater('contact in settings.contacts').row(1));
 *
 *       expect(firstRepeat.element(by.model('contact.value')).getAttribute('value')).toBe('408 555 1212');
 *       expect(secondRepeat.element(by.model('contact.value')).getAttribute('value')).toBe('john.smith@example.org');
 *
 *       firstRepeat.element(by.linkText('clear')).click();
 *
 *       expect(firstRepeat.element(by.model('contact.value')).getAttribute('value')).toBe('');
 *
 *       container.element(by.linkText('add')).click();
 *
 *       expect(container.element(by.repeater('contact in settings.contacts').row(2)).element(by.model('contact.value')).getAttribute('value')).toBe('yourname@example.org');
 *     });
 *   </file>
 * </example>
 *
 ...
 *
 *      expect(container.element(by.model('name')).getAttribute('value')).toBe('John Smith');
 *
 *      var firstRepeat = container.element(by.repeater('contact in contacts').row(0));
 *      var secondRepeat = container.element(by.repeater('contact in contacts').row(1));
 *
 *      expect(firstRepeat.element(by.model('contact.value')).getAttribute('value')).toBe('408 555 1212');
 *      expect(secondRepeat.element(by.model('contact.value')).getAttribute('value')).toBe('john.smith@example.org');
 *
 ...
 *      container.element(by.linkText('add')).click();
 *
 *      expect(container.element(by.repeater('contact in contacts').row(2)).element(by.model('contact.value')).getAttribute('value')).toBe('yourname@example.org');
 *    });
 *  </file>
 *</example>
 ...
 *   </file>
 *   <file name="script.js">
 *     angular.module('includeExample', ['ngAnimate']).controller('ExampleController', ['$scope', function($scope) {
 *       $scope.templates = [{
 *         name: 'template1.html',
 *         url: 'template1.html'
 *       }, {
 ...
 *   <file name="index.html">
 *     <script>
 *     angular.module('initExample', []).controller('ExampleController', ['$scope', function($scope) {
 *       $scope.list = [['a', 'b'], ['c', 'd']];
 *     }]);
 *     </script>
 *     <div ng-controller="ExampleController">
 ...
 *   <file name="index.html">
 *     <script>
 *       angular.module('pluralizeExample', []).controller('ExampleController', ['$scope', function($scope) {
 *         $scope.person1 = 'Igor';
 *         $scope.person2 = 'Misko';
 *         $scope.personCount = 1;
 *       }]);
 ...
 *       Without Offset:
 *       <ng-pluralize count="personCount" when="{'0': 'Nobody is viewing.', 'one': '1 person is viewing.', 'other': '{} people are viewing.'}">
 *       </ng-pluralize><br>
 *       
 *       <!-- Example with offset -->
 *       With Offset(2):
 *       <ng-pluralize count="personCount" offset=2 when="{'0': 'Nobody is viewing.',
 *               '1': '{{person1}} is viewing.',
 *               '2': '{{person1}} and {{person2}} are viewing.',
 *               'one': '{{person1}}, {{person2}} and one other person are viewing.',
 *               'other': '{{person1}}, {{person2}} and {} other people are viewing.'}">
...
var ngRepeatDirective = ['$parse', '$animate', function($parse, $animate) {
	...
	return {
		...
		link: function($scope, $element, $attr, ctrl, $transclude) {
			...
			$scope.$watchCollection(rhs, function(collection) {// watch props
				var index, length,
				...
				for (index = 0; index < length; index++) {
					...
					if (lastBlockMap.hasOwnProperty(trackById)) {
						...
						forEach(nextBlockOrder, function(block) {// restore lastBlockMap
 ...
 *     thumbsDown = element(by.css('span.glyphicon-thumbs-down'));
 *   
 ...
 *   </file>
 *   <file name="script.js">
 *     angular.module('switchExample', ['ngAnimate']).controller('ExampleController', ['$scope', function($scope) {
...
var ngSwitchDirective = ['$animate', function($animate) {
	return {
		restrict: 'EA',
		require: 'ngSwitch',
		controller: ['$scope', function() {// asks for $scope to fool the BC controller module
			this.cases = {};
		}],
 ...
 *   <file name="index.html">
 *     <script>
 *     angular.module('transcludeExample', []).directive('pane', function() {
 *       return {
 *         restrict: 'E',
 *         transclude: true,
 *         scope: {
 ...
 *   <file name="index.html">
 *     <script>
 *     angular.module('selectExample', []).controller('ExampleController', ['$scope', function($scope) {
 *       $scope.colors = [
	...
	return {
		...
		link: function(scope, element, attr, ctrls) {
			...
			function setupAsSingle(scope, selectElement, ngModelCtrl, selectCtrl) {
				ngModelCtrl.$render = function() {
					...
					if (selectCtrl.hasOption(viewValue)) {
						...
						if (viewValue === '') {// to make IE9 happy
			...
			function setupAsOptions(scope, selectElement, ctrl) {
				...
				function render() {// Temporary location for the option groups before we render them
					var optionGroups = {'':[]},
						optionGroupNames = [''],
						optionGroupName,
						optionGroup,
					...
					if (!multiple) {
						if (nullOption || modelValue === null) {// insert null option if we have a placeholder, or the model is null
							optionGroups[''].unshift({
								id: '',
								label: '',
								selected: !selectedSet
							});
						} else if (!selectedSet) {// option could not be found, we have to insert the undefined item
							optionGroups[''].unshift({
								id: '?',
								label: '',
								selected: true
					...
					for (groupIndex = 0, groupLength = optionGroupNames.length; groupIndex < groupLength; groupIndex++) {
						...
						for (index = 0, length = optionGroup.length; index < length; index++) {
							...
							if ((existingOption = existingOptions[index+1])) {
								...
								if (option.id === '' && nullOption) {// if it's a null option
									element = nullOption;// put back the pre-compiled element
								} else {
...
!window.angular.$$csp() && window.angular.element(document).find('head').prepend(['<style>',
	'@charset "UTF-8";',
	'[ng\\:cloak],',
	'[ng-cloak],',
	'[data-ng-cloak],',
