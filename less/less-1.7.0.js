...
less.Parser = function(env) {
	var input, // LeSS input string
		...
		parser, parsers, rootFilename = env && env.filename;
	...
	parser = {
		...
		parse: function(str, callback, additionalData) {
			...
			root.toCSS = (function(evaluate) {
				return function(options, variables) {
					...
					//
					//   new tree.Rule("@color", new tree.Value([
					//     new tree.Expression([
					//       new tree.Color("#f01")
					//     ])
					//    ]))
		...
		parsers: parsers = {
			...
			primary: function() {
				...
				while (current) {
					node = this.extendRule() || mixin.definition() || this.rule() || this.ruleset() || mixin.call() || this.comment() || this.rulesetCall() || this.directive();
					if (node) {
						root.push(node);
					} else {
						if (!($re(/^[\s\n]+/) || $re(/^;+/))) {
			...
			entities: {
				...
				//
				call: function() {
					var name, nameLC, args, alphaRet, index = i;
					...
					if (nameLC === "alpha") {
						alphaRet = parsers.alpha();
						if (typeof alphaRet !== "undefined") {
							return alphaRet;
						}
					}
			...
			mixin: {
				...
				args: function(isCall) {
					...
					while (true) {
						...
						if (isCall) {
							if (arg.value && arg.value.length == 1) {// Variable
								val = arg.value[0];
							}
						} else {
							val = arg;
			...
			rule: function(tryAnonymous) {
				...
				if (name) {
					...
					if (!value) {
						...
						value = !tryAnonymous && (env.compress || isVariable) ? (this.value() || this.anonymousValue()) : (this.anonymousValue() || this.value());
			...
			directive: function() {
				...
				*/
				...
				if (rules || (!hasBlock && value && $char(";"))) {
					forget();
					return new tree.Directive(name, value, rules, index, env.currentFileInfo, env.dumpLineNumbers ? getDebugInfo(index, input, env) : null);
				}
...
tree.functions = {
	...
	"data-uri": function(mimetypeNode, filePathNode) {
		...
		var mimetype = mimetypeNode.value,
			filePath = (filePathNode && filePathNode.value);
...
tree.Directive.prototype = {
	...
	eval: function(env) {
		...
		return new tree.Directive(this.name, value, rules, this.index, this.currentFileInfo, this.debugInfo);
	},
...
tree.Import.prototype = {
	...
	eval: function(env) {
		...
		if (this.options.inline) {//todo needs to reference css file not import
				...
				for (m = 0; m < candidates.length; m++) {
					...
					if ((candidate === defNone) || (candidate === defaultResult)) {
						try {
							...
							Array.prototype.push.apply(rules, mixin.evalCall(env, args, this.important).rules);
						} catch (e) {
							throw {
								message: e.message,
								index: this.index,
...
tree.Rule.prototype = {
	...
	eval: function(env) {
		...
		try {
			...
			return new tree.Rule(name, evaldValue, this.important, this.merge, this.index, this.currentFileInfo, this.inline);
		} catch (e) {
					...
					this._importer.push(importNode.getPath(), importNode.currentFileInfo, importNode.options, function(e, root, importedAtRoot, fullPath) {
						...
						if (root) {
							...
							if (!inlineCSS && (env.importMultiple || !duplicateImport)) {
								...
								new tree.importVisitor(importVisitor._importer, subFinish, env, importVisitor.onceFileDetectionMap, importVisitor.recursionDetector).run(root);
								return;
							}
						}
...
(function(tree) {
	...
	tree.sourceMapOutput.prototype.add = function(chunk, fileInfo, index, mapLines) {
		if (!chunk) {// ignore adding empty strings
			return;
		}
...
function createCSS(styles, sheet, lastModified) {
	...
	// If there is no oldCss, just append; otherwise, only append if we need to replace oldCss with an updated stylesheet
...
function doXHR(url, type, callback, errback) {
	var xhr = getXMLHttpRequest(),
		async = isFileProtocol ? less.fileAsync : less.async;
