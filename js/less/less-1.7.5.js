		...
		parsers: parsers = {
			...
			extend: function(isRule) {
				...
				do {
					...
					while (! (option = $re(/^(all)(?=\s*(\)|,))/))) {
						...
						if (elements) {
							elements.push(e); } else { elements = [ e ];
						}
					}
			...
			selector: function(isLess) {
				...
				while ((isLess && (extend = this.extend())) || (isLess && (when = $re(/^when/))) || (e = this.element())) {
					if (when) {
						...
						if (extendList) {
							extendList.push(extend); } else { extendList = [ extend ];
						}
					} else {
						if (extendList) {
			...
			ruleset: function() {
				...
				while (true) {
					...
					if (selectors) {
						selectors.push(s); } else { selectors = [ s ];
					}
					this.comments();
...
tree.mixin.Definition.prototype = {
	...
	matchCondition: function(args, env) {
		if (this.condition && !this.condition.eval(
			new tree.evalEnv(env, [this.evalParams(env, new tree.evalEnv(env, this.frames ? this.frames.concat(env.frames) : env.frames), args, [])] // the parameter variables
...
tree.Rule.prototype = {
	...
	eval: function(env) {
		...
		try {
			...
			return new tree.Rule(name, evaldValue, this.important, this.merge, this.index, this.currentFileInfo, this.inline, variable);
		} catch(e) {
...
tree.Ruleset.prototype = {
	...
	genCSS: function(env, output) {
		...
		function isRulesetLikeNode(rule, root) {
			if (rule.rules) {// if it has nested rules, then it should be treated like a ruleset
				return true;
			}
		...
		for (i = 0; i < this.rules.length; i++) {
			...
			} else {//charsets should float on top of everything
			...
			if (!mapLines) {
				...
				for (i = 0; i < lines.length; i++) {
					this._sourceMapGenerator.addMapping({
						...
						original: { line: sourceLines.length + i,
...
function pathDiff(url, baseUrl) {// diff between two paths to create a relative path
...
less.watch = function() {// Watch mode
	if (!less.watchMode) {
