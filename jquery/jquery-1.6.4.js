...
jQuery.fn = jQuery.prototype = {
	...
	init: function(selector, context, rootjQuery) {
		...
		if (selector === "body" && !context && document.body) {// The body element only exists once, optimize finding it
		...
		if (typeof selector === "string") {// Handle HTML strings
			...
			if (match && (match[1] || !context)) {
				if (match[1]) {// HANDLE: $(html) -> $(array)
					...
					if (elem && elem.parentNode) {
						...
						this.length = 1;// Otherwise, we inject the element directly into the jQuery object
						this[0] = elem;
					}
			...
			} else if (!context || context.jquery) {// HANDLE: $(expr, $(...))
				return (context || rootjQuery).find(selector);
			} else {// HANDLE: $(expr, context) (which is just equivalent to: $(context).find(expr)
				return this.constructor(context).find(selector);
			}
		} else if (jQuery.isFunction(selector)) {// HANDLE: $(function) Shortcut for document ready
	...
	jquery: "1.6.4",// The current version of jQuery being used
	
	length: 0,// The default length of a jQuery object is 0
	
	size: function() {// The number of elements contained in the matched element set
		return this.length;
	},
	...
	pushStack: function(elems, name, selector) {
		...
		ret.prevObject = this;// Add the old object onto the stack (as a reference)
		...
		return ret;// Return the newly-formed element set
	},
	...
	ready: function(fn) {
		jQuery.bindReady();// Attach the listeners
		readyList.done(fn);// Add the callback
...
jQuery.extend = jQuery.fn.extend = function() {
	...
	if (length === i) {// extend jQuery itself if only one argument is passed
		target = this;
		--i;
	}
...
jQuery.extend({
	...
	ready: function(wait) {
		...
		if ((wait === true && !--jQuery.readyWait) || (wait !== true && !jQuery.isReady)) {
			if (!document.body) {// Make sure body exists, at least, in case IE gets a little overzealous (ticket #5443).
				return setTimeout(jQuery.ready, 1);
			}
			
			jQuery.isReady = true;// Remember that the DOM is ready
			
			if (wait !== true && --jQuery.readyWait > 0) {// If a normal DOM Ready event fired, decrement, and wait if need be
				return;
			}
			
			readyList.resolveWith(document, [ jQuery ]);// If there are functions bound, to execute
			
			if (jQuery.fn.trigger) {// Trigger any bound ready events
	...
	bindReady: function() {
		...
		if (document.addEventListener) {
			document.addEventListener("DOMContentLoaded", DOMContentLoaded, false);// Use the handy event callback
			
			window.addEventListener("load", jQuery.ready, false);// A fallback to window.onload, that will always work
	...
	// Since version 1.3, DOM methods and functions like alert aren't supported. They return false on IE (#2968).
	...
	makeArray: function(array, results) {
		...
		if (array != null) {
			...
			// The extra typeof function check is to prevent crashes in Safari 2 (See: #3039)
	...
	map: function(elems, callback, arg) {
		...
		return ret.concat.apply([], ret);// Flatten any nested arrays
	},
	...
	_Deferred: function() {
		var // callbacks list
			...
			deferred = {
				...
				resolveWith: function(context, args) {// resolve with given context and args
					if (!cancelled && !fired && !firing) {// make sure args are available (#8421)
						args = args || [];
						firing = 1;
						try {
							while (callbacks[ 0 ]) {
				...
				resolve: function() {// resolve with this as context and given arguments
					deferred.resolveWith(this, arguments);
					return this;
				},
				
				isResolved: function() {// Has this deferred been resolved?
					return !!(firing || fired);
				},
...
jQuery.support = (function() {
	...
	if (!all || !all.length || !a) {// Can't get basic test support
		return {};
	}
...
jQuery.extend({
	...
	noData: {
		"embed": true,
		"object": "clsid:D27CDB6E-AE6D-11cf-96B8-444553540000",// Ban all objects except for Flash (which handle expandos)
		"applet": true
	},
	...
	data: function(elem, name, data, pvt /* Internal Use Only */) {
		...
		if (!id) {// Only DOM nodes need a new unique ID for each element since their data ends up in the global cache
			if (isNode) {
		...
		// not attempt to inspect the internal events object using jQuery.data, as this internal data object is undocumented and subject to change.
		...
		if (getByName) {
			ret = thisCache[ name ];// First Try to find as-is property data
			
			if (ret == null) {// Test for null|undefined property data
				ret = thisCache[ jQuery.camelCase(name) ];// Try to find the camelCased property
			}
		} else {
			ret = thisCache;
		}
	...
	removeData: function(elem, name, pvt /* Internal Use Only */) {
		...
		if (name) {
			...
			if (thisCache) {
				if (!thisCache[ name ]) {// Support interoperable removal of hyphenated or camelcased keys
		...
		if (pvt) {// See jQuery.data for more information
		...
		if (internalCache) {
			cache[ id ] = {};
			// TODO: This is a hack for 1.5 ONLY. Avoids exposing jQuery metadata on plain JS objects when the object is serialized using
			// JSON.stringify
			if (!isNode) {
				cache[ id ].toJSON = jQuery.noop;
			}
		...
		} else if (isNode) {// Otherwise, we need to eliminate the expando on the node to avoid false lookups in the cache for entries that no longer exist
			// IE does not allow us to delete expando properties from nodes,
			// nor does it have a removeAttribute function on Document nodes; we must handle all of these cases
	...
	valHooks: {
		...
		select: {
			get: function(elem) {
				...
				if (index < 0) {// Nothing was selected
					return null;
				}
				
				for (var i = one ? index : 0, max = one ? index + 1 : options.length; i < max; i++) {// Loop through all the selected options
	...
	attrHooks: {
		...
		value: {
			...
			set: function(elem, value, name) {
				...
				elem.value = value;// Does not return so that setAttribute is also used
			}
		}
	},
...
var Sizzle = function(selector, context, results, seed) {
	...
	} else {// Take a shortcut and set the context if the root selector is an ID (but not if it'll be faster if the inner selector is an ID)
...
} catch(e) {// Provide a fallback method if it does not work
	makeArray = function(array, results) {
		var i = 0,
			ret = results || [];
			...
			if (!Sizzle.isXML(node)) {
				try { 
					if (pseudoWorks || !Expr.match.PSEUDO.test(expr) && !/!=/.test(expr)) {
						...
						// As well, disconnected nodes are said to be in a document fragment in IE 9, so check for that
...
jQuery.fn.extend({
	load: function(url, params, callback) {
		...
		} else if (!this.length) {// Don't do a request if no elements are being requested
			return this;
		}
		...
		if (params) {// If the second parameter was provided
			if (jQuery.isFunction(params)) {// If it's a function
				callback = params;// We assume that it's the callback
				params = undefined;
		...
		var self = this;
		jQuery.ajax({// Request the remote document
			url: url,
			type: type,
			dataType: "html",
			data: params,
...
jQuery.each([ "Height", "Width" ], function(i, name) {
	...
	jQuery.fn[ "inner" + name ] = function() {// innerHeight and innerWidth
		var elem = this[0];
	...
	jQuery.fn[ type ] = function(size) {
		...
		if (elem.nodeType === 9) {// Get document width or height
		...
		if (size === undefined) {// Get or set width or height on the element
