...
// http://i18next.com
(function() {// add indexOf to non ECMA-262 standard compliant browsers
	...
	sync = {
			...
			if (!options.dynamicLoad) {
				...
				f.each(ns.namespaces, function(nsIndex, nsValue) {
					f.each(lngs, function(lngIndex, lngValue) {
						...
						if (typeof options.customLoad == 'function'){// Use the specified custom callback.
							options.customLoad(lngValue, nsValue, options, loadComplete);
						} else {//~ // Use our inbuilt sync.
		...
		postMissing: function(lng, ns, key, defaultValue, lngs) {
			...
			if (o.sendMissingTo === 'fallback' && o.fallbackLng[0] !== false) {
				for (i = 0; i < o.fallbackLng.length; i++) {
					urls.push({
						lng: o.fallbackLng[i],
	...
	function _ajax(options) {
		var getXhr = function(callback) {// v0.5.0 of https://github.com/goloroden/http.js
			if (window.XMLHttpRequest) {// Use the native XHR object if the browser supports it
				return callback(null, new XMLHttpRequest());
			}
			
			if (window.ActiveXObject) {
				try {// In Internet Explorer check for ActiveX versions of the XHR object
	...
	var f = {
		...
		toLanguages: function(lng) {
			var log = this.log,
				languages = [],
				whitelist = o.lngWhitelist || false,
			...
			if (typeof lng === 'string' && lng.indexOf('-') > -1) {
				...
				if (o.load !== 'current') {
					addLanguage(parts[this.getCountyIndexOfLng(lng)]);
				}
			} else {
				addLanguage(lng);
			}
			...
			for (var i = 0; i < o.fallbackLng.length; i++) {
				if (languages.indexOf(o.fallbackLng[i]) === -1 && o.fallbackLng[i]) {
	...
	function applyReuse(translated, options) {
		...
		while (translated.indexOf(o.reusePrefix) != -1) {
			...
			var index_of_opening = translated.lastIndexOf(o.reusePrefix),
				index_of_end_of_closing = translated.indexOf(o.reuseSuffix, index_of_opening) + o.reuseSuffix.length,
				token = translated.substring(index_of_opening, index_of_end_of_closing),
				token_without_symbols = token.replace(o.reusePrefix, '').replace(o.reuseSuffix, '');
