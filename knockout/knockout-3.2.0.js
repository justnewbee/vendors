...
(function(factory) {
	...
	} else if (typeof define === 'function' && define['amd']) {// [2] AMD anonymous module
...
ko.computed = ko.dependentObservable = function(evaluatorFunctionOrOptions, evaluatorFunctionTarget, options) {
	...
	function dependentObservable() {
		if (arguments.length > 0) {
			if (typeof writeFunction === "function") {// Writing a value
...
(function() {
	...
	var commentNodesHaveTextProperty = document && document.createComment("test").text === "<!--test-->",
		startCommentRegex = commentNodesHaveTextProperty ? /^<!--\s*ko(?:\s+([\s\S]+))?\s*-->$/ : /^\s*ko(?:\s+([\s\S]+))?\s*$/,
	...
	ko.virtualElements = {
		...
		prepend: function(containerNode, nodeToPrepend) {
			...
			} else {// Start comments must always have a parent and at least one following sibling (the end comment)
		...
		insertAfter: function(containerNode, nodeToInsert, insertAfterNode) {
			...
			} else if (!isStartComment(containerNode)) {// Insert after insertion point
				if (insertAfterNode.nextSibling) {
					containerNode.insertBefore(nodeToInsert, insertAfterNode.nextSibling);
				} else {
					containerNode.appendChild(nodeToInsert);
				}
			} else {// Children of start comments must always have a parent and at least one following sibling (the end comment)
	...
	ko.components.addBindingsForCustomElement = function(allBindings, node, bindingContext, valueAccessors) {
		...
		if (node.nodeType === 1) {
			...
			if (componentName) {
				...
				allBindings['component'] = valueAccessors ? function() { return componentBindingValue; } : componentBindingValue;
			}
		}
...
ko.bindingHandlers['options'] = {
	...
	'update': function(element, valueAccessor, allBindings) {
		...
		function applyToObject(object, predicate, defaultValue) {
			...
			if (predicateType == "string") {// Given a string; treat it as a property name on the data value
...
(function() {
			...
			if (preprocessNode) {
				invokeForEachNodeInContinuousRange(firstNode, lastNode, function(node, nextNodeInRange) {
					var nodePreviousSibling = node.previousSibling,
						newNodes = preprocessNode.call(provider, node);
	...
	function executeTemplate(targetNodeOrNodeArray, renderMode, template, bindingContext, options) {
		...
		var firstTargetNode = targetNodeOrNodeArray && getFirstNodeFromPossibleArray(targetNodeOrNodeArray),
			templateDocument = firstTargetNode && firstTargetNode.ownerDocument,
	...
	function resolveTemplateName(template, data, context) {
		...
		if (typeof template === 'function') {// 2. A function of (data, context) returning a string
	...
	ko.renderTemplate = function(template, dataOrBindingContext, options, targetNodeOrNodeArray, renderMode) {
		...
		if (targetNodeOrNodeArray) {
			...
			return ko.dependentObservable(function() {// So the DOM is automatically updated when any dependency changes
	...
	ko.expressionRewriting.bindingRewriteValidators['template'] = function(bindingValue) {
		...
		if ((parsedBindingValue.length == 1) && parsedBindingValue[0]['unknown']) {// It looks like a string literal, not an object literal, so treat it as a named template (which is allowed for rewriting)
			return null;
		}
		if (ko.expressionRewriting.keyValueArrayContainsKey(parsedBindingValue, "name")) {// Named templates can be rewritten, so return "no error"
			return null;
		}
...
(function() {
	...
	ko.utils.setDomNodeChildrenFromArrayMapping = function(domNode, array, mapping, options, callbackAfterAddingNodes) {
		...
		var isFirstExecution = ko.utils.domData.get(domNode, lastMappingResultDomDataKey) === undefined,
			...
			lastArray = ko.utils.arrayMap(lastMappingResult, function(x) {
				return x.arrayEntry;
			}),
