...
jQuery.extend({
	...
	access: function(elems, fn, key, value, chainable, emptyGet, pass) {
		...
		if (key && typeof key === "object") {// Sets many values
			for (i in key) {
			...
			if (bulk) {
				...
				} else {// Otherwise they run against the entire set
					fn.call(elems, value);
					fn = null;
				}
			}
 ...
 *     after the list has been fired right away with the latest "memorized" values (like a Deferred)
 *
	...
	var // Actual callback list
		...
		add = function(args) {
			...
			for (i = 0, length = args.length; i < length; i++) {
				...
				if (type === "array") {// Inspect recursively
					add(elem);
				} else if (type === "function") {// Add if not in unique mode and callback is not in
		...
		self = {
			...
			add: function() {
				if (list) {
					...
					if (firing) {
						firingLength = list.length;
					} else if (memory && memory !== true) {// With memory, if we're not firing then we should call right away, unless previous firing was halted (stopOnFalse)
						firingStart = length;
...
var getText = Sizzle.getText = function(elem) {
	...
	if (nodeType) {
		if (nodeType === 1 || nodeType === 9 || nodeType === 11) {// Use textContent || innerText for elements
...
if (document.documentElement.compareDocumentPosition) {
	...
	sortOrder = function(a, b) {
		if (a === b) {// The nodes are identical, we can exit early
			hasDuplicate = true;
			return 0;
		}
		
		if (a.sourceIndex && b.sourceIndex) {// Fallback to using sourceIndex (in IE) if it's available on both nodes
		...
		if (!aup) {// If no parents were found then the nodes are disconnected
			return -1;
		}
		...
		Sizzle = function(query, context, extra, seed) {
			...
			if (!seed && !Sizzle.isXML(context)) {
				...
				if (match && (context.nodeType === 1 || context.nodeType === 9)) {
					if (match[1]) {// Speed-up: Sizzle("TAG")
						return makeArray(context.getElementsByTagName(query), extra);
					}
					
					if (match[2] && Expr.find.CLASS && context.getElementsByClassName) {// Speed-up: Sizzle(".CLASS")
				...
				if (context.nodeType === 9) {
					...
					if (match && match[3]) {// Speed-up: Sizzle("#ID")
...
(function() {
	...
	if (matches) {
		...
		try {// This should fail with an exception, Gecko does not error, returns false instead
...
jQuery.fn.extend({
	...
	index: function(elem) {
		...
		return jQuery.inArray(elem.jquery ? elem[0] : elem, this);// If it receives a jQuery object, the first element is used
	},
...
function cloneFixAttributes(src, dest) {
	...
	} else if (nodeName === "script" && dest.text !== src.text) {// IE blanks contents when cloning scripts
		dest.text = src.text;
	}
...
function findInputs(elem) {
	...
	} else if (nodeName !== "script" && typeof elem.getElementsByTagName !== "undefined") {// Skip scripts, get other children
...
jQuery.extend({
	...
	css: function(elem, name, extra) {
		...
		if (curCSS) {// Otherwise, if a way to get the computed value exists, use that
...
var r20 = /%20/g,
	...
	ajaxLocation,// Document location
	
	ajaxLocParts,// Document location segments
...
jQuery.fn.extend({
	load: function(url, params, callback) {
		...
		if (!this.length) {// Don't do a request if no elements are being requested
			return this;
		}
...
jQuery.extend({
	...
	ajaxSettings: {
		...
		*/
		...
		// For options that shouldn't be deep extended: you can add your own custom options here if
		// and when you create one that shouldn't be deep extended (see ajaxExtend)
		flatOptions: {
			context: true,
			url: true
		}
...
function buildParams(prefix, obj, traditional, add) {
	if (jQuery.isArray(obj)) {
		jQuery.each(obj, function(i, v) {// Serialize array item
		...
		for (var name in obj) {// Serialize object item
...
function ajaxConvert(s, response) {
	...
	for (i = 1; i < length; i++) {
		...
		if (current === "*") {
			current = prev;
		} else if (prev !== "*" && prev !== current) {// If no auto and dataTypes are actually different
			// Get the converter
			conversion = prev + " " + current;
			conv = converters[conversion] || converters["* " + current];
			
			if (!conv) {// If there is no direct converter, search transitively
				conv2 = undefined;
			...
			if (!(conv || conv2)) {// If we found no converter, dispatch an error
				jQuery.error("No conversion from " + conversion.replace(" "," to "));
			}
			
			if (conv !== true) {// If found converter is not an equivalence
				response = conv ? conv(response) : conv2(conv1(response));// Convert with 1 or 2 converters accordingly
			}
		}
	}
	return response;
