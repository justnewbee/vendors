...
function minErr(module, ErrorConstructor) {
	...
	return function() {
		...
		message = message + '\nhttp://errors.angularjs.org/1.3.0-rc.3/' + (module ? module + '/' : '') + code;
 ...
 *         I can add: {{a}} + {{b}} = {{ a+b }}
 *         <p>This renders because the controller does not fail to instantiate, by using explicit annotation style (see script.js for details)</p>
 *       </div>
 ...
 *         I can add: {{a}} + {{b}} = {{ a+b }}
 *         <p>The controller could not be instantiated, due to relying on automatic function annotations (which are disabled in strict mode). As such, the content of this section is not interpolated, and there should be an error in your web console.</p>
 *       </div>
 *     </div>
 *   </file>
 *   <file name="script.js">
 *     angular.module('ngAppStrictDemo', []).controller('BadController', function($scope) {
...
function jqLiteAddNodes(root, elements) {// THIS CODE IS VERY HOT. Don't make changes without benchmarking.
	if (elements) {// if a Node (the most common case)
		if (elements.nodeType) {
			...
			if (typeof length === 'number' && elements.window !== elements) {// if an Array or NodeList and not a Window
				if (length) {
 ...
 * by converting the function into a string using `toString()` method and extracting the argument names.
 * ```js
 *   // Given
 ...
 *   </file>
 *   <file name="script.js">
 *     angular.module('anchorScrollExample', []).controller('ScrollController', ['$scope', '$location', '$anchorScroll', function($scope, $location, $anchorScroll) {
...
var $AnimateProvider = ['$provide', function($provide) {
	 ...
	 * animation object which contains callback functions for each event that is expected to be animated.
	 *
	 *   * `eventFn`: `function(Element, doneFunction)` The element to animate, the `doneFunction`
	 *   must be called once the element animation is complete. If a function is returned then the
	 *   animation service will use this function to cancel the animation whenever a cancel event is triggered.
	 *
	 *
	 * ```js
	 *   return {
 ...
 * @param {number} maxPriority only apply directives lower than given priority (Only effects the root element(s), not their children)
...
function $CompileProvider($provide, $$sanitizeUriProvider) {
	...
	this.$get = ['$injector', '$interpolate', '$exceptionHandler', '$templateRequest', '$parse', '$controller', '$rootScope', '$document', '$sce', '$animate', '$$sanitizeUri', function($injector, $interpolate, $exceptionHandler, $templateRequest, $parse, $controller, $rootScope, $document, $sce, $animate, $$sanitizeUri) {
		...
		function compile($compileNodes, transcludeFn, maxPriority, ignoreDirective, previousCompileContext) {
			...
			return function(scope, cloneConnectFn, transcludeControllers, parentBoundTranscludeFn, futureParentElement) {
				...
				var $linkNode = cloneConnectFn ? JQLitePrototype.clone.call(namespaceAdaptedCompileNodes) /* IMPORTANT!!! */: namespaceAdaptedCompileNodes;
		 ...
		 * @param {Object=} originalReplaceDirective An optional directive that will be ignored when compiling the transclusion.
		...
		function addTextInterpolateDirective(directives, text) {
			...
			if (interpolateFn) {
				directives.push({
					priority: 0,
					compile: function(templateNode) {// text interpolate compile
		...
		function addAttrInterpolateDirective(node, directives, value, name, allOrNothing) {
			...
			directives.push({
				...
				compile: function() {
					return {
						pre: function(scope, element, attr) {// attr interpolate preLink
		 ...
		 * @param {JqLite} elementsToRemove The jqLite element which we are going to replace. We keep the shell, but replace its DOM node reference.
...
function $HttpProvider() {
	 ...
	 *
	 * @returns {boolean|Object} If a value is specified, returns the $httpProvider for chaining. otherwise, returns the current configured value.
	 */
	...
	this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector', function($httpBackend, $browser, $cacheFactory, $rootScope, $q, $injector) {
		 ...
		 * `transformRequest` and/or `transformResponse` properties on the configuration object passed into `$http`.
		 *
		 ...
		 *       <button id="samplejsonpbtn" ng-click="updateModel('JSONP', 'https://angularjs.org/greet.php?callback=JSON_CALLBACK&name=Super%20Hero')">Sample JSONP</button>
		 *       <button id="invalidjsonpbtn" ng-click="updateModel('JSONP', 'https://angularjs.org/doesntexist&callback=JSON_CALLBACK')">Invalid JSONP</button>
...
function LocationHashbangUrl(appBase, hashPrefix) {
	...
	this.$$parse = function(url) {
		...
		function removeWindowsDriveName (path, url, base) {
			/*
			 * Matches paths for file protocol on windows, such as /C:/foo/bar, and captures only /foo/bar.
			 */
	...
	this.$get = ['$filter', '$sniffer', function($filter, $sniffer) {
		...
		function expressionInputDirtyCheck(newValue, oldValueOfValue) {
			...
			if (typeof newValue === 'object') {
				...
				// TODO(docs): add a note to docs that by implementing valueOf even objects and arrays can be cheaply dirty-checked
		...
		Scope.prototype = {
			...
			$new: function(isolate) {
				...
				if (isolate) {
					...
					// Only create a child scope class if somebody asks for one, but cache it to allow the VM to optimize lookups
					if (!this.$$ChildScope) {
 ...
 *
 * <file name="script.js">
 *  angular.module('mySceApp', ['ngSanitize']).controller('AppController', ['$http', '$templateCache', '$sce', function($http, $templateCache, $sce) {
 *    var self = this;
...
function $SceProvider() {
	...
	this.$get = ['$parse', '$sniffer', '$sceDelegate', function($parse, $sniffer, $sceDelegate) {
		 ...
		 * @returns {*} An object that can be passed to {@link ng.$sce#getTrustedHtml $sce.getTrustedHtml(value)} to obtain the original value.
 ...
 *
 * Expressions, like the one defined for the `ngClick` directive in the example below,
 * are evaluated with respect to the current scope. Therefore, there is no risk of inadvertently
 * coding in a dependency on a global value in such an expression.
 *
 * @example
 ...
 *
 * ```js
 *   it('should be the same instance', inject(function($filterProvider) {
 ...
 *       letterLimitInput = element(by.model('letterLimit')),
 *       longNumberLimitInput = element(by.model('longNumberLimit')),
 *       limitedNumbers = element(by.binding('numbers | limitTo:numLimit')),
 *       limitedLetters = element(by.binding('letters | limitTo:letterLimit')),
...
var inputType = {
	 ...
	 *       valid = element(by.binding('myForm.input.$valid')),
	 ...
	 *    var scr = "var ipt = document.getElementById('exampleInput'); " + "ipt.value = '" + val + "';" + "angular.element(ipt).scope().$apply(function(s) { s.myForm[ipt.name].$setViewValue('" + val + "'); });";
	 ...
	 * <file name="index.html">
	 *   <script>
	 *     angular.module('dateExample', []).controller('DateController', ['$scope', function($scope) {
	 ...
	 *   <file name="index.html">
	 *     <script>
	 *       angular.module('timeExample', []).controller('DateController', ['$scope', function($scope) {
	 *         $scope.value = new Date(1970, 0, 1, 14, 57, 0);
	 *       }]);
	 *     </script>
	 *     <form name="myForm" ng-controller="DateController as dateCtrl">
	 *       Pick a between 8am and 5pm:
	 *       <input type="time" id="exampleInput" name="input" ng-model="value" placeholder="HH:mm:ss" min="08:00:00" max="17:00:00" required />
	 *       <span class="error" ng-show="myForm.input.$error.required">Required!</span>
	 ...
	 *   <file name="index.html">
	 *     <script>
	 *     angular.module('weekExample', []).controller('DateController', ['$scope', function($scope) {
	 *       $scope.value = new Date(2013, 0, 3);
	 *     }]);
	 *     </script>
	 *     <form name="myForm" ng-controller="DateController as dateCtrl">
	 *       Pick a date between in 2013:
	 *       <input id="exampleInput" type="week" name="input" ng-model="value" placeholder="YYYY-W##" min="2012-W32" max="2013-W52" required />
	 *       <span class="error" ng-show="myForm.input.$error.required">Required!</span>
	 *       <span class="error" ng-show="myForm.input.$error.week">Not a valid date!</span>
	 ...
	 *  <file name="index.html">
	 *    <script>
	 *      angular.module('monthExample', []).controller('DateController', ['$scope', function($scope) {
	 *        $scope.value = new Date(2013, 9, 1);
	 *      }]);
	 *    </script>
	 *    <form name="myForm" ng-controller="DateController as dateCtrl">
	 *      Pick a month int 2013:
	 *      <input id="exampleInput" type="month" name="input" ng-model="value" placeholder="yyyy-MM" min="2013-01" max="2013-12" required />
	 *      <span class="error" ng-show="myForm.input.$error.required">Required!</span>
	 *      <span class="error" ng-show="myForm.input.$error.month">Not a valid month!</span>
...
function baseInputType(scope, element, attr, ctrl, $sniffer, $browser) {
	var validity = element.prop(VALIDITY_STATE_PROPERTY),
		placeholder = element[0].placeholder, noevent = {},
 ...
 *         <span class="error" ng-show="myForm.lastName.$error.maxlength">Too long!</span><br>
 *       </form>
 *       <hr>
 *       <tt>user = {{user}}</tt><br/>
 ...
 *   return $http.get('/api/users/' + value).then(function() {
...
var NgModelController = ['$scope', '$exceptionHandler', '$attrs', '$element', '$parse', '$animate', '$timeout', '$rootScope', '$q', '$interpolate', function($scope, $exceptionHandler, $attr, $element, $parse, $animate, $timeout, $rootScope, $q, $interpolate) {
	...
	var ngModelSet = function(newValue) {
		var getterSetter;
	 ...
	 * @param {boolean} isValid Whether the current state is valid (true), invalid (false), pending (undefined), or skipped (null).
	 */
	addSetValidityMethod({
		ctrl: this,
		$element: $element,
 ...
 *       debug = element(by.binding('confirmed'));
 *
 ...
 *       <form name="userForm">
 *         Name:
 *         <input type="text" name="userName" ng-model="user.name" ng-model-options="{ updateOn: 'blur' }" ng-keyup="cancel($event)" /><br />
 *         
 *         Other data:
 ...
 * <example>
 *   <file name="index.html">
 *    <button ng-mousedown="count = count + 1" ng-init="count=0">Increment (on mouse down)</button>
 *    count: {{count}}
 *   </file>
 * </example>
 */
 ...
 * <example>
 *   <file name="index.html">
 *    <button ng-mouseup="count = count + 1" ng-init="count=0">Increment (on mouse up)</button>
 *    count: {{count}}
 *   </file>
 * </example>
 */
 ...
 * <example>
 *   <file name="index.html">
 *    <button ng-mouseenter="count = count + 1" ng-init="count=0">Increment (when mouse enters)</button>
 *    count: {{count}}
 *   </file>
 * </example>
 */
 ...
 * <example>
 *   <file name="index.html">
 *    <button ng-mouseleave="count = count + 1" ng-init="count=0">Increment (when mouse leaves)</button>
 *    count: {{count}}
 *   </file>
 * </example>
 */
 ...
 * <example>
 *   <file name="index.html">
 *    <button ng-mousemove="count = count + 1" ng-init="count=0">Increment (when mouse moves)</button>
 *    count: {{count}}
 *   </file>
 * </example>
 */
 ...
 *     Show when checked:
 *     <span ng-if="checked" class="animate-if">I'm removed when the checkbox is unchecked.</span>
 *   </file>
	...
	return {
		...
		link: function(scope, $element, $attr, ctrl) {
			if (/SVG/.test($element[0].toString())) {
				...
				// support innerHTML, so detect this here and try to generate the contents specially.
				$element.empty();
		...
		compile: function($element, $attr) {
			...
			return function($scope, $element, $attr, ctrl, $transclude) {
				...
				$scope.$watchCollection(rhs, function(collection) {
					var index, length,
						previousNode = $element[0],// node that cloned nodes should be inserted after initialized to the comment node anchor
						nextNode,
					...
					for (index = 0; index < collectionLength; index++) {
						...
						if (block.scope) {
							...
							do {// skip nodes that are already pending removal via leave animation
