	...
	function testDOMProps(props, obj, elem) {
		for (var i in props) {
			...
			if (item !== undefined) {
				...
				if (is(item, 'function')) {
					return item.bind(elem || obj);// default to autobind unless override
				}
		...
		Modernizr['inputtypes'] = (function(props) {
			for (var i = 0, bool, inputElemType, defaultView, len = props.length; i < len; i++) {
				...
				if (bool) {
					...
					} else if (/^(url|email)$/.test(inputElemType)) {// Interestingly, opera fails the earlier test, so it doesn't even make it here.
	...
	(function(window, document) {
		...
		function shivMethods(ownerDocument, data) {
			...
			ownerDocument.createElement = function(nodeName) {
				if (!html5.shivMethods) {// abort shiv
