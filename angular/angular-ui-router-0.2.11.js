...
function UrlMatcher(pattern, config) {
	...
	while ((m = placeholder.exec(pattern))) {
		...
		if (segment.indexOf('?') >= 0) {// we're into the search part
			break;
		}
 ...
 * @example
 * <pre>
 * new UrlMatcher('/user/{id}?q').format({
 *   id:'bob',
 *   q:'yes'
 * });
...
function $UrlRouterProvider($locationProvider, $urlMatcherFactory) {
	...
	function $get($location, $rootScope, $injector, $browser) {
		...
		return {
			 ...
			 * @example
			 * <pre>
			 * angular.module('app', ['ui.router']).run(function($rootScope, $urlRouter) {
...
function $StateProvider($urlRouterProvider, $urlMatcherFactory) {
	...
	function $get($rootScope, $q, $view, $injector, $resolve, $stateParams, $urlRouter) {
		 ...
		 * @example
		 * <pre>
		 * expect($state.href("about.person", {
		 *   person: "bob"
		 * })).toEqual("/about/bob");
		 * </pre>
		 *
	...
	return {
		...
		link: function(scope, element, attrs, uiSrefActive) {
			var ref = parseStateRef(attrs.uiSref, $state.current.name),
				params = null,
				url = null, base = stateContext(element) || $state.$current,
				isForm = element[0].nodeName === "FORM",
				attr = isForm ? "action" : "href", nav = true,
				options = {
					relative: base,
					inherit: true
				},
