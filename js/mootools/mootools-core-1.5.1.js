...
local.setDocument = function(document) {
	...
	if (features.isHTMLDocument) {
		...
		if (testNode.getElementsByClassName) {
			try {// Safari 3.2 getElementsByClassName caches results
			...
			try {// Opera 9.6 getElementsByClassName doesnt detects the class if its not the first one
		...
		if (testNode.querySelectorAll) {
			try {// IE 8 returns closed nodes (EG:"</foo>") for querySelectorAll('*') for some documents
				testNode.innerHTML = 'foo</foo>';
			...
			try {// Webkit and Opera dont return selected options on querySelectorAll
				testNode.innerHTML = '<select><option selected="selected">a</option></select>';
				features.brokenCheckedQSA = (testNode.querySelectorAll(':checked').length == 0);
			} catch(ex) {}
			
			try {// IE returns incorrect results for attr[*^$]="" selectors on querySelectorAll
				testNode.innerHTML = '<a class=""></a>';
				features.brokenEmptyAttributeQSA = (testNode.querySelectorAll('[class*=""]').length != 0);
			} catch(ex) {}
		}
		
		try {// IE6-7, if a form has an input of id x, form.getAttribute(x) returns a reference to the input
		...
		if (features.nativeMatchesSelector) {
			try {// if matchesSelector trows errors on incorrect sintaxes we can use it
...
local.search = function(context, expression, append, first) {
	...
	if (context.navigator) {// Convert the node from a window to a document
...
local.parseNTHArgument = function(argument) {
	...
	parsed = (special == 'n') ? {
		a: a,
		b: b
	} : (special == 'odd') ? {a: 2, b: 1} : (special == 'even') ? {
		a: 2,
		b: 0
	} : {
		a: 0,
	...
	' ': function(node, tag, id, classes, attributes, pseudos, classList) {// all child nodes, any level
		...
		if (this.isHTMLDocument) {
			getById: if (id) {
				...
				if (this.document !== node && !this.contains(node, item)) {
					return;
				}
...
Element.implement({
	...
	toQueryString: function() {
		...
		this.getElements('input, select, textarea').each(function(el) {
			...
			var value = (el.get('tag') == 'select') ? el.getSelected().map(function(opt) {// IE
