...
(function(window, undefined) {
	...
	less.Parser = function(env) {
		...
		function skipWhitespace(length) {
			var oldi = i, oldj = j, curr = i - currentPos, endIndex = i + current.length - curr, mem = (i += length), inp = input, c;
		...
		function LessError(e, env) {
			var input = getInput(e, env), loc = getLocation(e.index, input), line = loc.line, col = loc.column, callLine = e.call && getLocation(e.call, input).line, lines = input.split('\n');
		...
		parser = {
			...
			parse: function(str, callback, additionalData) {
				...
				chunks = (function(input) {
					var len = input.length, level = 0, parenLevel = 0, lastOpening, lastOpeningParen, lastMultiComment, lastMultiCommentEndBrace, chunks = [], emitFrom = 0, parserCurrentIndex, currentChunkStartIndex, cc, cc2, matched;
				...
				root.toCSS = (function(evaluate) {
					return function(options, variables) {
						options = options || {};
						var evaldRoot, css, evalEnv = new tree.evalEnv(options);
						...
						try {
							var preEvalVisitors = [], visitors = [new tree.joinSelectorVisitor(), new tree.processExtendsVisitor(), new tree.toCSSVisitor({
						...
						if (options.cleancss && less.mode === 'node') {
							var CleanCSS = require('clean-css'), cleancssOptions = options.cleancssOptions || {};
				...
				if (i < input.length - 1) {
					...
					error = {
						...
						line: line,
						column: loc.column,
						extract: [lines[line - 2], lines[line - 1], lines[line]]
					};
				}
			...
			parsers: parsers = {
				...
				primary: function() {
					...
					while (current) {
						node = this.extendRule() || mixin.definition() || this.rule() || this.ruleset() || mixin.call() || this.comment() || this.directive();
						if (node) {
							root.push(node);
						} else {
							if (!($re(/^[\s\n]+/) || $re(/^;+/))) {
				...
				entities: {
					...
					quoted: function() {
						...
						if (input.charAt(j) === '~') {// Escaped strings
							j++;
							e = true;
						}
					...
					url: function() {
						...
						value = this.quoted() || this.variable() || $re(/^(?:(?:\\[\(\)'"])|[^\(\)'"])+/) || "";
				...
				mixin: {
					...
					//
					call: function() {
						var s = input.charAt(i), important = false, index = i, elemIndex, elements, elem, e, c, args;
					...
					args: function(isCall) {
						var parsers = parser.parsers, entities = parsers.entities, returner = {
							args: null,
							variadic: false
						}, expressions = [], argsSemiColon = [], argsComma = [], isSemiColonSeperated, expressionContainsNamed, name, nameLoop, value, arg;
						...
						while (true) {
							if (isCall) {
								...
								if (input.charAt(i) === '.' && $re(/^\.{3}/)) {
									...
									(isSemiColonSeperated ? argsSemiColon : argsComma).push({
										variadic: true
									});
									break;
								}
					...
					definition: function() {
						...
						if ((input.charAt(i) !== '.' && input.charAt(i) !== '#') || peek(/^[^{]*\}/)) {
							return;
						}
				...
				entity: function() {
					...
					return entities.literal() || entities.variable() || entities.url() || entities.call() || entities.keyword() || entities.javascript() || this.comment();
				},
				...
				element: function() {
					var index = i,
						c = this.combinator();
						e = $re(/^(?:\d+\.\d+|\d+)%/) || $re(/^(?:[.#]?|:*)(?:[\w-]|[^\x00-\x9f]|\\(?:[A-Fa-f0-9]{1,6} ?|[^A-Fa-f0-9]))+/) || $char('*') || $char('&') || this.attribute() || $re(/^\([^()@]+\)/) || $re(/^[\.#](?=@)/) || this.entities.variableCurly(),
						v;
				...
				rule: function(tryAnonymous) {
					...
					if (name) {
						...
						value = !tryAnonymous && (env.compress || (name.charAt && (name.charAt(0) === '@'))) ? (this.value() || this.anonymousValue()) : (this.anonymousValue() || this.value());
				...
				//     @charset "utf-8";
				directive: function() {
					var index = i, name, value, rules, nonVendorSpecificName, hasBlock, hasIdentifier, hasExpression, identifier;
				...
				condition: function() {
					var entities = this.entities, index = i, negate = false, a, b, c, op;
				...
				operand: function() {
					var entities = this.entities, p = input.charAt(i + 1), negate;
				...
				ruleProperty: function() {
					...
					if ((name.length > 1) && match(/^\s*(\+?)\s*:/)) {
						...
						for (k = 0; k < name.length; k++) {
							s = name[k];
							name[k] = (s.charAt(0) !== '@') ? new tree.Keyword(s) : new tree.Variable('@' + s.slice(2, -1), index[k], env.currentFileInfo);
						}
						return name;
					}
				}
	...
	(function(tree) {
		tree.functions = {
			...
			hsva: function(h, s, v, a) {
				...
				var vs = [v, v * (1 - s), v * (1 - f * s), v * (1 - (1 - f) * s)], perm = [[0, 3, 1], [2, 0, 1], [1, 0, 3], [1, 2, 0], [3, 1, 0], [0, 1, 2]];
			...
			mix: function(color1, color2, weight) {
				...
				var p = weight.value / 100.0,
					w = p * 2 - 1, a = color1.toHSL().a - color2.toHSL().a,
			...
			'%': function(quoted /* arg, arg, ...*/) {
				var args = Array.prototype.slice.call(arguments, 1), str = quoted.value;
			...
			_minmax: function(isMin, args) {
				...
				var i, j, current, currentUnified, referenceUnified, unit, order = [], // elems only contains original argument values.
				values = {};// key is the unit.toString() for unified tree.Dimension values, value is the index into the order array.
			...
			color: function(n) {
				if (n instanceof tree.Quoted) {
					var colorCandidate = n.value, returnColor;
			...
			"data-uri": function(mimetypeNode, filePathNode) {
				...
				var fs = require('fs'), path = require('path'), useBase64 = false;
				...
				var DATA_URI_MAX_KB = 32, fileSizeInKB = parseInt((buf.length / 1024), 10);
			...
			"svg-gradient": function(direction) {
				...
				returner = '<?xml version="1.0" ?>' + '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="100%" height="100%" viewBox="0 0 1 1" preserveAspectRatio="none">' + '<' + gradientType
						+ 'Gradient id="gradient" gradientUnits="userSpaceOnUse" ' + gradientDirectionSvg + '>';
		...
		tree._mime = {
			...
			lookup: function(filepath) {
				var ext = require('path').extname(filepath), type = tree._mime._types[ext];
				if (type === undefined) {
	...
	(function(tree) {
		...
		tree.Anonymous.prototype = {
			...
			compare: function(x) {
				...
				var left = this.toCSS(), right = x.toCSS();
		...
		tree.Call.prototype = {
			...
			eval: function(env) {
				var args = this.args.map(function(a) {
					return a.eval(env);
				}), nameLC = this.name.toLowerCase(), result, func;
				
				if (nameLC in tree.functions) {
					try {
	...
	(function(tree) {
		...
		tree.Color.prototype = {
			...
			toCSS: function(env, doNotCompress) {
				var compress = env && env.compress && !doNotCompress, alpha = tree.fround(env, this.alpha);
			...
			operate: function(env, op, other) {
				var rgb = [], alpha = this.alpha * (1 - other.alpha) + other.alpha;
			...
			toHSL: function() {
				var r = this.rgb[0] / 255, g = this.rgb[1] / 255, b = this.rgb[2] / 255, a = this.alpha, max = Math.max(r, g, b), min = Math.min(r, g, b), h, s, l = (max + min) / 2, d = max - min;
			...
			toHSV: function() {
				var r = this.rgb[0] / 255, g = this.rgb[1] / 255, b = this.rgb[2] / 255, a = this.alpha, max = Math.max(r, g, b), min = Math.min(r, g, b), h, s, v = max;
	...
	(function(tree) {
		...
		tree.Comment.prototype = {
			...
			isSilent: function(env) {
				var isReference = (this.currentFileInfo && this.currentFileInfo.reference && !this.isReferenced), isCompressed = env.compress && !this.value.match(/^\/\*!/);
	...
	(function(tree) {
		...
		tree.Condition.prototype = {
			...
			eval: function(env) {
				var a = this.lvalue.eval(env), b = this.rvalue.eval(env);
	...
	(function(tree) {
		...
		tree.Dimension = function(value, unit) {
			...
			this.unit = (unit && unit instanceof tree.Unit) ? unit : new tree.Unit(unit ? [unit] : undefined);
		};
		...
		tree.Dimension.prototype = {
			...
			genCSS: function(env, output) {
				...
				var value = tree.fround(env, this.value), strValue = String(value);
			...
			operate: function(env, op, other) {
				...
				var value = tree.operate(env, op, this.value, other.value), unit = this.unit.clone();
			...
			compare: function(other) {
				if (other instanceof tree.Dimension) {
					var a = this.unify(), b = other.unify(), aValue = a.value, bValue = b.value;
	...
	(function(tree) {
		...
		tree.Element.prototype = {
			...
			eval: function(env) {
				return new tree.Element(this.combinator, this.value.eval ? this.value.eval(env) : this.value, this.index, this.currentFileInfo);
			},
			genCSS: function(env, output) {
		...
		tree.Attribute.prototype = {
			type: "Attribute",
			eval: function(env) {
				return new tree.Attribute(this.key.eval ? this.key.eval(env) : this.key, this.op, (this.value && this.value.eval) ? this.value.eval(env) : this.value);
			},
			genCSS: function(env, output) {
				output.add(this.toCSS(env));
			},
	...
	(function(tree) {
		tree.Extend = function(selector, option, index) {
	...
	(function(tree) {
		...
		tree.Media.prototype = {
			...
			eval: function(env) {
				...
				return env.mediaPath.length === 0 ? media.evalTop(env) : media.evalNested(env);
			},
			variable: function(name) {
				return tree.Ruleset.prototype.variable.call(this.rules[0], name);
			},
			...
			emptySelectors: function() {
				var el = new tree.Element('', '&', this.index, this.currentFileInfo), sels = [new tree.Selector([el], null, null, this.index, this.currentFileInfo)];
			...
			evalNested: function(env) {
				var i, value, path = env.mediaPath.concat([this]);
				
				// Extract the media-query conditions separated with `,` (OR).
				for (i = 0; i < path.length; i++) {
					value = path[i].features instanceof tree.Value ? path[i].features.value : path[i].features;
	...
	(function(tree) {
		...
		tree.mixin.Call.prototype = {
			...
			eval: function(env) {
				var mixins, mixin, args, rules = [], match = false, i, m, f, isRecursive, isOneFound, rule, candidates = [], candidate, conditionResult = [], defaultFunc = tree.defaultFunc, defaultResult, defNone = 0, defTrue = 1, defFalse = 2, count;
						...
						for (m = 0; m < candidates.length; m++) {
							...
							if ((candidate === defNone) || (candidate === defaultResult)) {
								try {
									...
									Array.prototype.push.apply(rules, mixin.eval(env, args, this.important).rules);
								} catch (e) {
									throw {
										message: e.message,
										index: this.index,
		...
		tree.mixin.Definition.prototype = {
			...
			evalParams: function(env, mixinEnv, args, evaldArguments) {
				/*jshint boss:true */
				var frame = new tree.Ruleset(null, null), varargs, arg, params = this.params.slice(0), i, j, val, name, isNamedFound, argIndex;
			...
			eval: function(env, args, important) {
				var _arguments = [], mixinFrames = this.frames.concat(env.frames), frame = this.evalParams(env, new tree.evalEnv(env, mixinFrames), args, _arguments), rules, ruleset;
			...
			matchCondition: function(args, env) {
				...
				if (this.condition && !this.condition.eval(new tree.evalEnv(env, [this.evalParams(env, new tree.evalEnv(env, this.frames.concat(env.frames)), args, [])] .concat(this.frames) .concat(env.frames)))) { 
					return false;
				}
				return true;
			},
	...
	(function(tree) {
		...
		tree.Operation.prototype = {
			...
			eval: function(env) {
				var a = this.operands[0].eval(env), b = this.operands[1].eval(env);
	...
	(function(tree) {
		...
		tree.Rule.prototype = {
			...
			eval: function(env) {
				...
				try {
					return new tree.Rule(name, this.value.eval(env), this.important, this.merge, this.index, this.currentFileInfo, this.inline);
				} catch (e) {
			...
			makeImportant: function() {
				return new tree.Rule(this.name, this.value, "!important", this.merge, this.index, this.currentFileInfo, this.inline);
			}
		};
	...
	(function(tree) {
		...
		tree.Ruleset.prototype = {
			...
			eval: function(env) {
				var thisSelectors = this.selectors, selectors, selCnt, i, defaultFunc = tree.defaultFunc;
				...
				var rules = this.rules ? this.rules.slice(0) : null, ruleset = new tree.Ruleset(selectors, rules, this.strictImports), rule, subRule;
			...
			matchCondition: function(args, env) {
				...
				if (lastSelector.condition && !lastSelector.condition.eval(new tree.evalEnv(env, env.frames))) {
					return false;
				}
			...
			rulesets: function() {
				...
				var _Ruleset = tree.Ruleset, _MixinDefinition = tree.mixin.Definition, filtRules = [], rules = this.rules, cnt = rules.length, i, rule;
			...
			find: function(selector, self) {
				self = self || this;
				var rules = [], match, key = selector.toCSS();
						...
						for (var j = 0; j < rule.selectors.length; j++) {
							...
							if (match) {
								if (selector.elements.length > match) {
									Array.prototype.push.apply(rules, rule.find(new tree.Selector(selector.elements.slice(match)), self));
								} else {
									rules.push(rule);
								}
								break;
			...
			genCSS: function(env, output) {
				var i, j, ruleNodes = [], rulesetNodes = [], rulesetNodeCnt, debugInfo, // Line number debugging
				rule, path;
				...
				var tabRuleStr = env.compress ? '' : Array(env.tabLevel + 1).join("  "), tabSetStr = env.compress ? '' : Array(env.tabLevel).join("  "), sep;
	...
	(function(tree) {
		...
		tree.Selector.prototype = {
			...
			eval: function(env) {
				var evaldCondition = this.condition && this.condition.eval(env), elements = this.elements, extendList = this.extendList;
	...
	(function(tree) {
		var parseCopyProperties = ['paths',// option - unmodified - paths to search for imports on
		...
		var evalCopyProperties = ['silent',// whether to swallow errors and warnings
	...
	(function(tree) {
		...
		tree.importVisitor.prototype = {
			...
			visitImport: function(importNode, visitArgs) {
				var importVisitor = this, evaldImportNode, inlineCSS = importNode.options.inline;
						...
						this._importer.push(importNode.getPath(), importNode.currentFileInfo, importNode.options, function(e, root, imported, fullPath) {
							...
							if (root) {
								...
								if (!inlineCSS && !importNode.skip) {
									new tree.importVisitor(importVisitor._importer, subFinish, env).run(root);
									return;
								}
							}
		...
		tree.processExtendsVisitor.prototype = {
			...
			isElementValuesEqual: function(elementValue1, elementValue2) {
				...
				if (elementValue1 instanceof tree.Attribute) {
					...
					if (!elementValue1.value || !elementValue2.value) {
						return !(elementValue1.value || elementValue2.value);
					}
			...
			extendSelector: function(matches, selectorPath, replacementSelector) {
				...
				var currentSelectorPathIndex = 0,
					...
					path = [],
					matchIndex, selector, firstElement, match, newElements;
	...
	less.env = less.env || (location.hostname == '127.0.0.1' || location.hostname == '0.0.0.0' || location.hostname == 'localhost' || (location.port && location.port.length > 0) || isFileProtocol ? 'development' : 'production');
	...
	function extractId(href) {
		// Remove protocol & domainRemove root / ➔ Remove simple extension ➔ Replace illegal characters ➔ Replace dots with colons(for valid id)
		return href.replace(/^[a-z-]+:\/+?[^\/]+/, '').replace(/^\//, '').replace(/\.[a-zA-Z]+$/, '').replace(/[^\.\w-]+/g, '-').replace(/\./g, ':');
	}
	...
	function errorConsole(e, rootHref) {
		var template = '{line} {content}',
			...
			errors = [],
			content = (e.type || "Syntax") + "Error: " + (e.message || 'There is an error in your .less file') + " in " + filename + " ";
	...
	function createCSS(styles, sheet, lastModified) {
		...
		if (css.styleSheet) {// IE
			...
			keepOldCss = (oldCss !== null && oldCss.childNodes.length > 0 && css.childNodes.length > 0 && oldCss.firstChild.nodeValue === css.firstChild.nodeValue);
		}
	...
	function errorHTML(e, rootHref) {
		var id = 'less-error-message:' + extractId(rootHref || ""),
			...
			elem = document.createElement('div'), timer, content, errors = [],
			filename = e.filename || rootHref,
			filenameNoPath = filename.match(/([^\/]+(\?.*)?)$/)[1];
		...
		createCSS(['.less-error-message ul, .less-error-message li {',
		...
		elem.style.cssText = ["font-family: Arial, sans-serif",
	...
	function loadStyles(modifyVars) {
		...
		for (var i = 0; i < styles.length; i++) {
			...
			if (style.type.match(typePattern)) {
				var env = new less.tree.parseEnv(less), lessText = style.innerHTML || '';
	...
	function pathDiff(url, baseUrl) {
		...
		var urlParts = extractUrlParts(url), baseUrlParts = extractUrlParts(baseUrl), i, max, urlDirectories, baseUrlDirectories, diff = "";
	...
	function getXMLHttpRequest() {
		...
		try {
			return new ActiveXObject("MSXML2.XMLHTTP.3.0");// global ActiveXObject
		} catch (e) {
	...
	function doXHR(url, type, callback, errback) {
		var xhr = getXMLHttpRequest(), async = isFileProtocol ? less.fileAsync : less.async;
	...
	function loadFile(originalHref, currentFileInfo, callback, env, modifyVars) {
		...
		var hrefParts = extractUrlParts(originalHref, window.location.href), href = hrefParts.url, newFileInfo = {
	...
	function loadStyleSheet(sheet, callback, reload, remaining, modifyVars) {
		...
		loadFile(sheet.href, null, function(e, data, path, newFileInfo, webInfo) {
			if (webInfo) {
				...
				if (!reload && timestamp && webInfo.lastModified && (new Date(webInfo.lastModified).valueOf() === new Date(timestamp).valueOf())) {
					// Use local copy
					createCSS(css, sheet);
					webInfo.local = true;
