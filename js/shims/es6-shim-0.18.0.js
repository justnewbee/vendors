	...
	var main = function() {
		...
		var ES = {
			...
			IsCallable: function(x) {
				return typeof x === 'function' && _toString.call(x) === '[object Function]';// some versions of IE say that typeof /abc/ === 'function'
			},
			...
			ToLength: function(value) {
				...
				if (len <= 0) {// includes converting -0 to +0
					return 0;
				}
			...
			Construct: function(C, args) {
				...
				if (ES.IsCallable(C['@@create'])) {
					obj = C['@@create']();
				} else {// OrdinaryCreateFromConstructor
		...
		defineProperties(String, {
			...
			raw: function(callSite) { // raw.length===1
				var substitutions = _slice.call(arguments, 1, arguments.length),
					cooked = ES.ToObject(callSite, 'bad callSite'),
					rawValue = cooked.raw,
					raw = ES.ToObject(rawValue, 'bad raw value'),
		...
		if (String.fromCodePoint.length !== 1) {
			...
			defineProperty(String, 'fromCodePoint', function(_) {
		...
		var StringShims = {
			...
			endsWith: function(searchStr) {
				...
				var thisLen = thisStr.length,
					...
					pos = posArg === undefined ? thisLen : ES.ToInteger(posArg),
		...
		if (!startsWithIsCompliant) {// Firefox has a noncompliant startsWith implementation
		...
		var PromiseShim = (function() {
			...
			var _promiseAllResolver = function(index, values, capability, remaining) {
				var done = false;
				return function(x) {
					if (done) {// protect against being called multiple times
						return;
					}
					done = true;
					values[index] = x;
		...
		if (supportsDescriptors) {
			var fastkey = function fastkey(key) {
				...
				if (type === 'number') {// note that -0 will get coerced to "0" when used as a property key
					return key;
				}
			...
			var collectionShims = {
				Map: (function() {
					...
					defineProperties(Map, {
						'@@create': function(obj) {
							var constructor = this,
								prototype = constructor.prototype || Map$prototype;
				...
				Set: (function() {
					...
					defineProperties(SetShim, {
						'@@create': function(obj) {
							var constructor = this,
								prototype = constructor.prototype || Set$prototype;
