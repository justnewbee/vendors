...
function $StateProvider($urlRouterProvider, $urlMatcherFactory, $locationProvider) {
	...
	function equalForKeys(a, b, keys) {
		...
		if (!keys) {
			keys = [];
			for (var n in a) {// Used instead of Object.keys() for IE8 compatibility
				keys.push(n);
			}
		}
	...
	return {
		...
		link: function(scope, element, attrs) {
			var ref = parseStateRef(attrs.uiSref),
				...
				nav = true,
				stateData = element.parent().inheritedData('$uiView');
	...
	function when(url, route) {
		...
		if (route.redirectTo != null) {
			...
			if (isString(redirect)) {
				...
				handler = function(params, $location) {// Adapt to $urlRouterProvider API
			...
			$stateProvider.state(inherit(route, {// Regular route, configure as state
				parent: null,
